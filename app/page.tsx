...
function HomeView({ setView }: { setView: (view: 'home' | 'admin') => void }) {
  ...
  const conversation = useConversation({
    ...
    clientTools: {
      generate_solar_report: async ({ address, email, phone }) => {
        try {
          // 1. Evaluate Roof
